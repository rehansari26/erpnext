...
def execute(filters: dict | None = None):
    ...
    # Get period list for periodicity
    if filters.get("filter_based_on") == "Date Range":
        period_list = get_period_list(
            ...
            company=filters.company[0] if filters.company else None,
        )
    else:
        period_list = get_period_list(
    ...
    if period_list:
        filters.from_date = period_list[0]["from_date"]
        filters.to_date = period_list[-1]["to_date"]
    
    columns = get_columns_with_periods(period_list, filters)
...
def validate_filters(filters):
    ...
    # Set fiscal_year for tb_validate_filters compatibility
    if filters.get("filter_based_on") == "Fiscal Year":
        filters.fiscal_year = filters.get("from_fiscal_year")
    else:
        # For date range, get fiscal year from the start date
        if filters.get("period_start_date"):
            from erpnext.accounts.utils import get_fiscal_year
            filters.fiscal_year = get_fiscal_year(filters.get("period_start_date"))[0]
...
def get_columns_with_periods(period_list, filters):
    """Get columns including period columns for periodicity"""
    columns = [
        {
            "fieldname": "account",
            "label": _("Account"),
    ...
    # Add total column if not yearly
    if filters.get("periodicity", "Yearly") != "Yearly":
        columns.append({
            "fieldname": "total",
            "label": _("Total"),
            "fieldtype": "Currency",
...
def get_data(filters, period_list) -> list[list]:
    ...
    for company in filters.company:
        ...
        tb_data = get_company_wise_tb_data(company_filter, reporting_currency, ignore_reporting_currency, period_list)
        consolidate_trial_balance_data(data, tb_data)

    for d in data:
        # Calculate totals for each row across periods
        total = 0
        for period in period_list:
            total += d.get(period.key, 0)
        if filters.get("periodicity", "Yearly") != "Yearly":
            d["total"] = total
            
        # Skip prepare_opening_closing for period-based data

    total_row = calculate_total_row(data, reporting_currency, period_list)
...
def get_company_wise_tb_data(filters, reporting_currency, ignore_reporting_currency, period_list):
    ...
    for period in period_list:
        period_filter = frappe._dict(filters)
        period_filter.from_date = period.from_date
        period_filter.to_date = period.to_date
    ...
    accumulate_period_values_into_parents(accounts, accounts_by_name, period_list)

    data = prepare_companywise_tb_data(accounts, filters, parent_children_map, reporting_currency, period_list)
...
def prepare_companywise_tb_data(accounts, filters, parent_children_map, reporting_currency, period_list):
    data = []
    ...
    for d in accounts:
        ...
        for period in period_list:
            ...
            if abs(row[period.key]) >= get_zero_cutoff(reporting_currency):
                has_value = True
...
def calculate_total_row(data, reporting_currency, period_list):
    total_row = {
        "account": "'" + _("Total") + "'",
    ...
    for d in data:
        if not d.get("parent_account"):
            for period in period_list:
                total_row[period.key] += d.get(period.key, 0.0)
...
def calculate_values_for_period(
    accounts,
    gl_entries_by_account,
    opening_balances,
    show_net_values,
    ...
    """Calculate values for a specific period"""
    for account in accounts:
        account_entries = gl_entries_by_account.get(account.name, [])
        debit = credit = 0.0
        ...
        opening = flt(opening_balances.get(account.name, 0.0))
        closing = opening + net_change
        
        # Store period-specific values
        account[period_key] = closing if show_net_values else net_change


def accumulate_period_values_into_parents(accounts, accounts_by_name, period_list):
    """Accumulate period values into parent accounts"""
    for d in reversed(accounts):
        if d.parent_account:
            for period in period_list:
                if period.key not in accounts_by_name[d.parent_account]:
                    accounts_by_name[d.parent_account][period.key] = 0.0
                accounts_by_name[d.parent_account][period.key] += d.get(period.key, 0.0)
